...
import { difficultyToRoundCount, oddOneOutDefinition } from '@cerebro-play/games';
...
export default function OddOneOutHarnessPage() {
  ...
  const recommendedDifficulty = useDifficultyRecommendation(user?.id, oddOneOutDefinition.skill);
  ...
  function start() {
    ...
    const engine = new GameEngine(oddOneOutDefinition, sessionId);
  ...
  const data = content?.data as OddOneOutData | undefined;
    ...
    >
      {phase === 'idle' && (
        <div className="flex-1 flex flex-col items-center justify-center gap-md text-center">
          <p className="font-body text-body-md text-on-surface-variant">Spot the number that breaks the pattern.</p>
          <DifficultyPicker
            value={difficulty}
            onChange={(value) => {

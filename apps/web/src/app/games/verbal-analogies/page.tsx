...
import { difficultyToRoundCount, verbalAnalogiesDefinition } from '@cerebro-play/games';
...
export default function VerbalAnalogiesHarnessPage() {
  ...
  const recommendedDifficulty = useDifficultyRecommendation(user?.id, verbalAnalogiesDefinition.skill);
  ...
  function start() {
    ...
    const engine = new GameEngine(verbalAnalogiesDefinition, sessionId);
        ...
        completeGameSession(
          {
            ...
            gameId: verbalAnalogiesDefinition.id,
  ...
  const data = content?.data as VerbalAnalogyData | undefined;
  const level = user ? calculateLevel(user.xp) : null;

  return (
    <GameShell
      gameName="Verbal Analogies"
      headerRight={
        phase === 'answering' ? (
      ...
      {phase === 'idle' && (
        <div className="flex-1 flex flex-col items-center justify-center gap-md text-center">
          <p className="font-body text-body-md text-on-surface-variant">
            A is to B as C is to ___. Find the matching relationship.
          </p>
          <DifficultyPicker
            value={difficulty}
            onChange={(value) => {

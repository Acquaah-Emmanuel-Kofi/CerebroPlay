...
import { difficultyToRoundCount, mentalMathSprintDefinition } from '@cerebro-play/games';
...
export default function MentalMathSprintHarnessPage() {
  ...
  const recommendedDifficulty = useDifficultyRecommendation(user?.id, mentalMathSprintDefinition.skill);
  ...
  function start() {
    ...
    const engine = new GameEngine(mentalMathSprintDefinition, sessionId);
        ...
        completeGameSession(
          {
            ...
            gameId: mentalMathSprintDefinition.id,
  ...
  const data = content?.data as MentalMathData | undefined;
    ...
    >
      {phase === 'idle' && (
        <div className="flex-1 flex flex-col items-center justify-center gap-md text-center">
          <p className="font-body text-body-md text-on-surface-variant">Solve each problem as fast as you can.</p>
          <DifficultyPicker
            value={difficulty}
            onChange={(value) => {
      ...
      {phase === 'answering' && data && (
        <div className="flex-1 flex flex-col items-center justify-center gap-md">
          ...
          <p className="font-display text-headline-md text-on-surface text-center">{content?.prompt}</p>
          <div className="grid grid-cols-2 gap-md w-full">
            {data.options.map((option) => (
              <button
                ...
                className="h-14 bg-surface-container-lowest border-2 border-surface-container-highest hover:border-primary rounded-xl font-label-bold text-label-bold text-on-surface transition-colors"
              >
                {option}
              </button>
            ))}
